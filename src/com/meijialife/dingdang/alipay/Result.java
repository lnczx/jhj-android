...
public class Result {
	...
	static {
		...
		sResultStatus.put("4003", "您的支付宝账户被冻结或不允许支付");//该用户绑定的支付宝账户被冻结或不允许支付
		...
		sResultStatus.put("6001", "取消支付");	//用户中途取消支付操作
	...
	private  boolean checkSign(String result) {
		...
		try {
			...
			e.printStackTrace();
			LogOut.i("Result", "Exception =" + e);
		}
		LogOut.i("Result", "checkSign =" + retVal);
		return retVal;
	}
